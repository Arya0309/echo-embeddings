...
# main.py
# Usage examples:
#   python main.py --json /path/echo_toy_triplets.json --model mistralai/Mistral-7B-Instruct-v0.1 --mode classical --pool mean
#   python main.py --json /path/echo_toy_triplets.json --model mistralai/Mistral-7B-Instruct-v0.1 --mode echo --pool mean
#   python main.py --json /path/echo_toy_triplets.json --mode echo --pool last --max-length 300
...
def main():
    ...
    for ex in examples:
        ...
        if struct == "S1":
            ...
            s1_pos_scores.append(sim_pos)
            s1_neg_scores.append(sim_neg)
        elif struct == "S2":
            total_s2 += 1
    ...
    with open(f"summary_{args.mode}_{args.pool}.csv", "w", encoding="utf-8") as fout:
        fout.write("mode,pool,model,subset,accuracy,n\n")
        fout.write(
            f"{args.mode},{args.pool},{args.model},ALL,{safe_acc(corr, total):.4f},{total}\n"
        )
        fout.write(
            f"{args.mode},{args.pool},{args.model},S1,{safe_acc(corr_s1, total_s1):.4f},{total_s1}\n"
        )
        fout.write(
            f"{args.mode},{args.pool},{args.model},S2,{safe_acc(corr_s2, total_s2):.4f},{total_s2}\n"
        )
    # import numpy as np

    # with open("s1_scores.json", "w") as fout:
    #     json.dump({"s1_pos": s1_pos_scores, "s1_neg": s1_neg_scores}, fout)
    # with open("s2_scores.json", "w") as fout:
    #     json.dump({"s2_pos": s2_pos_scores, "s2_neg": s2_neg_scores}, fout)
